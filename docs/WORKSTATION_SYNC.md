...
Treat the repository as the primary continuity source. We are currently on branch codex/archive-ux-reset, post-Sprint-20, and the current checkpoint is the archive-alignment UX reset with property targeting plus selected payment/issue detail focus complete in Rent & Issues.
```
