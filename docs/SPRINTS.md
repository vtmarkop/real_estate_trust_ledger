...
- Post-Sprint-20 archive-alignment UX reset status: the reset is underway on branch `codex/archive-ux-reset`, with the first slices restoring property targeting, strict `Daily work` versus read-only `History` separation, compact payment/issue detail dropdowns in `Rent & Issues`, and clearer tenant-side versus landlord-side score wording while preserving the rebuilt dispute/reviewer/scoring architecture
